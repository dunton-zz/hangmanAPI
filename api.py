...
CANCEL_GAME_REQUEST = endpoints.ResourceContainer(urlsafe_game_key=messages.StringField(1),)  # NOQA
...
class HangmanApi(remote.Service):
    ...
    def not_allowed(self, guess):
        '''Make sure only one letter is guessed at a time'''
    ...
    def make_move(self, request):
        ...
        if (self.not_allowed(request.guess)):
            raise endpoints.BadRequestException('Guess is not a single letter')
        ...
        if request.guess not in game_word:
            ...
            for (index, letter) in enumerate(game_word):
                if letter == request.guess:
                    game.word_so_far = game.word_so_far[0:index] +
                    request.guess + game.word_so_far[index + 1:]
    ...
    def get_user_games(self, request):
        ...
        user = User.query(User.name == request.user_name).get()
        if not user:
            raise endpoints.NotFoundException('''A user with that name
                                              does not exist'''')

        # attempt to match user and user key
        games = Game.query(Game.user == user.key and Game.game_over == False)  # NOQA
        return GameForms(items=[game.to_form("A user's game") for game
                                in games])
    ...
    def _cache_average_attempts():
        ...
        if games:
            ...
            memcache.set(MEMCACHE_MOVES_REMAINING,
                         '''The average moves remaining
                         is {:.2f}'''.format(average))
