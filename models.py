...
class Game(ndb.Model):
    ...
    def new_game(cls, user, word_urlsafe_key):
        ...
        #user_key = ndb.Key(urlsafe=user_urlsafe_key)
        ...
        # May need to check on game.user to game.user.key
        
        #game_id = Game.allocate_ids(size=1, parent=user_key)[0]
        #game.key = ndb.Key(Game, game_id, parent=user_key)
        game.history = []
        game.put()
        return game
    ...
    def end_game(self, won=False):
        ...
        # NEWEST ADDITON CHECK TO MAKE SURE IT LETS YOU FINISH GAME STILL
        # AND CHECK POINTS SYSTEM
...
class Score(ndb.Model):
    ...
    def to_form(self):
        return ScoreForm(user=self.user.get().name, won=self.won,
                         date=str(self.date), attempts_remaining=self.attempts_remaining,
...
class GameForm(messages.Message):
    ...
    #letters_guessed = messages.StringField(6)
